...
async function getFiles(absoluteDir, extensions, ignore, _path = '', _nested = false, layouts = [], fallbacks = []) {
    const list = []
    ...
    files = files
        ...
        .map(route => {
            ...
            if (!route.isLayout) {
                ...
                route.ranking = route.path.split('/').map(str => str.match(/(\[|\])|fallback/) ? 'A' : 'Z').join('')
...
function getRegex(str) {
    ...
    str = '^' + str.replace(MATCH_BRACKETS, '(?!index$)([^/]+)')
    return str
}
